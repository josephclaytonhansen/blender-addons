...
class ToggleSilhouetteViewOperator(bpy.types.Operator):
    ...
    bl_description = (
        "Toggle the silhouette view (all objects are white against a black background)"
    )
...
def draw_toggle_button(self, context):
    ...
    if addon_prefs.show_matte_button:
        layout.operator(
            ToggleSilhouetteViewOperator.bl_idname, text="Silhouette", icon="MOD_MASK"
        )
...
from bpy.types import Panel, PropertyGroup, UIList, AddonPreferences
...
class SRCCT_Preferences(AddonPreferences):
    ...
    sound: bpy.props.BoolProperty(
        name="Play render complete notification sound", default=False
    )
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.prop(
            self, "shading_rig_precise_editing", text="Precise Numerical Link Editing"
        )
...
class SR_OT_OpenDocumentation(bpy.types.Operator):
    ...
    def execute(self, context):
        webbrowser.open(
            "https://josephclaytonhansen.github.io/blender-addons/shading-rig-quick-start/"
        )
        return {"FINISHED"}
...
class SR_OT_OpenMultikeyDocumentation(bpy.types.Operator):
    ...
    def execute(self, context):
        webbrowser.open(
            "https://josephclaytonhansen.github.io/blender-addons/multikey-quick-start/"
        )
        return {"FINISHED"}
...
class SR_RigItem(PropertyGroup):
    ...
    parent_object: PointerProperty(
        ...
        in {"MESH", "CURVE", "EMPTY", "ARMATURE", "LIGHT", "CAMERA"},
    ...
    def draw(self, context):
        ...
        if scene.shading_rig_show_defaults:
            ...
            if (
                ...
                and scene.shading_rig_chararacter_name != ""
            ):
                row = col.row(align=True)
        ...
        if "ShadingRigEffect" not in bpy.data.node_groups:
            col.operator(
                setup_helpers.SR_OT_AppendNodes.bl_idname,
                icon="APPEND_BLEND" if addon_prefs.show_icons else "NONE",
            )

        if (
            not scene.shading_rig_chararacter_name
            or scene.shading_rig_chararacter_name == ""
        ):
            col.label(
                text="Set a rig name", icon="INFO" if addon_prefs.show_icons else "NONE"
            )
        else:
            if scene.shading_rig_default_light == None:
                col.label(
                    text="Set a default light",
                    icon="INFO" if addon_prefs.show_icons else "NONE",
                )

            if context.active_object and context.active_object.type == "MESH":
                col.operator(
                    setup_helpers.SR_OT_SetupObject.bl_idname,
                    icon="MATERIAL_DATA" if addon_prefs.show_icons else "NONE",
                )
                if len(scene.shading_rig_list) <= 0:
                    col.operator(
                        "shading_rig.list_add",
                        icon="ADD" if addon_prefs.show_icons else "NONE",
                    )
            else:
                col.label(
        ...
        if len(scene.shading_rig_list) <= 0:
            ...
            col.operator(
                externaldata_helpers.SR_OT_ClearCombinedData.bl_idname,
                icon="TRASH" if addon_prefs.show_icons else "NONE",
            )
        ...
        if scene.shading_rig_list and 0 <= scene.shading_rig_list_index < len(
            ...
            if active_item.show_active_settings:
                ...
                row = col.row(align=True)
                row.label(
                    text="", icon="MATERIAL" if addon_prefs.show_icons else "NONE"
                )
                ...
                if not addon_prefs.auto_apply_sr_presets:
                    preset_row.operator(
                        SR_OT_ApplyPreset.bl_idname, text="Apply Preset"
                    )
            ...
            col.operator(
                ...
                text=(
                    "Save Changes"
                    if not bpy.types.Scene.is_evaluating_shading_rig
            ...
            if active_item.correlations_index >= 0 and len(active_item.links) > 0:
                ...
                if (
                    not bpy.types.Scene.is_evaluating_shading_rig
                    and addon_prefs.shading_rig_precise_editing
                ):
...
def update_shading_rig_handler(scene, depsgraph):
    ...
    if bpy.types.Scene.is_evaluating_shading_rig:
        ...
        for rig_item in scene.shading_rig_list:
            ...
            if (
                rig_item.last_empty_name
                and rig_item.last_empty_name != current_empty_name
            ):
            ...
            if prev_transform:
                ...
                curr_rot_pos = list(current_light_rotation) + list(
                    current_light_position
                )
                # Use a more reasonable threshold for detecting changes (0.001 instead of 1e-5)
                if all(abs(a - b) < 0.001 for a, b in zip(rot_pos, curr_rot_pos)):
                    continue
            ...
            try:
                ...
                _previous_light_transforms[light_obj_key][
                    0
                ] = current_light_rotation.copy()
                _previous_light_transforms[light_obj_key][
                    1
                ] = current_light_position.copy()

            except Exception as e:
                if addon_prefs.debug_mode:
                    print(
                        f"Shading Rig Debug: Error in calculateWeightedEmptyPosition: {e}"
                    )
                continue
        ...
        if scene.shading_rig_list_index < 0 or scene.shading_rig_list_index >= len(
            scene.shading_rig_list
        ):
            return

        rig_item = scene.shading_rig_list[scene.shading_rig_list_index]
        if rig_item.correlations_index < 0 or rig_item.correlations_index >= len(
            rig_item.links
        ):
            return
        ...
        if previous_active_corr is None:
            ...
            if addon_prefs.debug_mode:
                print(
                    f"Shading Rig Debug: Correlation changed from {previous_active_corr.name if previous_active_corr else 'None'} to {active_corr.name}"
                )
        ...
        if correlation_changed:
            ...
            if previous_active_corr and rig_item.light_object and rig_item.empty_object:
                try:
                    ...
                    if addon_prefs.debug_mode:
                        print(
                            f"Shading Rig Debug: Saving PREVIOUS correlation '{previous_active_corr.name}' - Light pos: {current_light_pos}"
                        )
                        print(
                            f"Shading Rig Debug:  Saving PREVIOUS correlation '{previous_active_corr.name}' - Empty pos: {current_empty_pos}"
                        )
                    ...
                    if addon_prefs.debug_mode:
                        print(
                            f"Shading Rig Debug: Error saving previous correlation: {e}"
                        )
            ...
            try:
                if addon_prefs.debug_mode:
                    print(
                        f"Shading Rig Debug: Loading NEW correlation '{active_corr.name}' - Light pos: {active_corr.light_position}"
                    )
                    print(
                        f"Shading Rig Debug: Loading NEW correlation '{active_corr.name}' - Empty pos: {active_corr.empty_position}"
                    )
                ...
                if addon_prefs.debug_mode:
                    print(
                        f"Shading Rig Debug: Successfully loaded correlation '{active_corr.name}' values to objects"
                    )
                ...
                if addon_prefs.debug_mode:
                    print(
                        "Shading Rig Debug: Successfully synced to JSON after correlation change"
                    )
            ...
            if active_corr and rig_item.light_object and rig_item.empty_object:
                try:
                    ...
                    light_moved = any(
                        abs(a - b) > 0.01
                        for a, b in zip(current_light_pos, active_corr.light_position)
                    )
                    empty_moved = any(
                        abs(a - b) > 0.01
                        for a, b in zip(current_empty_pos, active_corr.empty_position)
                    )

                    if light_moved or empty_moved:
                        if addon_prefs.debug_mode:
                            print(
                                f"Detected significant movement, updating correlation '{active_corr.name}'"
                            )

                        active_corr.light_position = (
                            rig_item.light_object.location.copy()
                        )
                        active_corr.light_rotation = (
                            rig_item.light_object.rotation_euler.copy()
                        )
                        active_corr.empty_position = (
                            rig_item.empty_object.location.copy()
                        )
                        active_corr.empty_scale = rig_item.empty_object.scale.copy()
                        active_corr.empty_rotation = (
                            rig_item.empty_object.rotation_euler.copy()
                        )
                    ...
                    if addon_prefs.debug_mode:
                        print(
                            f"Shading Rig Debug: Error updating current correlation: {e}"
                        )


@persistent
def render_post(self):
    sound = bpy.context.preferences.addons[
        "shading-rig-and-cel-character-tools"
    ].preferences.sound
    if (
        bpy.context.scene.frame_current == bpy.context.scene.frame_end
        or bpy.context.scene.frame_start == bpy.context.scene.frame_end
    ):
        if sound:
            device = aud.Device()
            sound = aud.Sound(
